...
SELECT brand_name, name 
    ...
    -- What is the difference between putting the year in ''s and not. 
    -- The solution doesn't have year in ''s. 
...
-- left joins and inner joins.
 ...
 SELECT m.name, m.brand_name, b.name
    FROM Models AS m
      LEFT JOIN Brands AS b
        ON b.name = m.brand_name;

-- Inner joins brings all of the data from both tables together and allows you to select data from both tables. 
-- Only fields that exist (or are able to be matched in both tables with another feild in the other table) in both tables are showed. 
-- Left joins do what inner joins PLUS include data from all feilds that exist in the first table specified. Even if there is no corresponding
-- data to should for a row in the second table referenced. 
...
ALTER TABLE Awards ADD COLUMN winner_model_id VARCHAR(5);
